...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!token) return NextResponse.json({ error: 'Log in before using the armory.' }, { status: 401 });

    const body = await request.json().catch(() => ({}));
    const supabase = createAuthDatabaseClient();
    if (!supabase) return NextResponse.json({ error: 'The campaign database is not connected yet.' }, { status: 503 });

    const { data, error } = await supabase.rpc('run_armory_action', {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Armory work failed.' }, { status: 500 });
  }
}
